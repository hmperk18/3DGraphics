...
class Model{
    ...
    setVertexArrays(gl, shader){
        ...
        // TODO: bind texture coordinate buffer
        gl.bindBuffer(gl.ARRAY_BUFFER, this.mesh.texcoordBuffer);
        // TODO: enable texture coordinate vertex attribute array
        gl.enableVertexAttribArray(shader.attribLoc.texcoord);
        // TODO: set vertex attribute pointer for texture coordinates.
    ...
    setUniformData(shader, camera, lightingData){
        ...
        if(shader.uniformLoc.spotlight)
            gl.uniform3fv(shader.uniformLoc.spotlight, lightingData.spotlight.toFloat32())
        if(shader.uniformLoc.spotlight_dir)
            gl.uniform3fv(shader.uniformLoc.spotlight_dir, lightingData.spotlight_dir.toFloat32())
        if(shader.uniformLoc.spotlight_color)
            gl.uniform3fv(shader.uniformLoc.spotlight_color, lightingData.spotlight_color.toFloat32())
        if(shader.uniformLoc.spotlight2)
            gl.uniform3fv(shader.uniformLoc.spotlight2, lightingData.spotlight2.toFloat32())
        if(shader.uniformLoc.spotlight_dir2)
            gl.uniform3fv(shader.uniformLoc.spotlight_dir2, lightingData.spotlight_dir2.toFloat32())
        if(shader.uniformLoc.spotlight_color2)
            gl.uniform3fv(shader.uniformLoc.spotlight_color2, lightingData.spotlight_color2.toFloat32())
        if(shader.uniformLoc.spotlight_limit)
            gl.uniform1f(shader.uniformLoc.spotlight_limit, lightingData.spotlight_limit)
        
            // texturing
        // TODO: set texture data for main texture to texture unit 0
        // and main texture uniform of the fragment shader.
        if(shader.uniformLoc.mainTexture){
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, TextureCache[this.material.mainTexture]);
            gl.uniform1i(shader.uniformLoc.mainTexture, 0);
        }
