...
class PerspectiveCamera {
    ...
    */
    getPosition(){
        return this.cameraMatrix.multV3(Vector3.ZERO);
    }
    ...
    */
    getViewDir(){
        return this.cameraMatrix.transformV3(Vector3.BACK);
    }
}
...
class OrbiterCameraController {
    ...
    */
    constructor(canvas, camera, target, distance, zoomSpeed, rotationSpeed, movementSpeed, startingPitch, tartingYaw){
        ...
        this.rotationSpeed = rotationSpeed || 700;
    ...
    */
    update(){
      // if(this.yaw<0){
      //   this.yaw -= Math.sin((Time.time * Math.PI * 2) * 0.1) * (this.rotationSpeed / this.canvas.height);
      // }
      // if(this.yaw>0){
      //   this.yaw += Math.sin((Time.time * Math.PI * 2) * 0.1) * (this.rotationSpeed / this.canvas.height);
      // }
      ...
      //this.yaw -= Math.sin((Time.time * Math.PI * 2) * 0.1) * (this.rotationSpeed / this.canvas.height);
        ...
        if(Input.keyHold("arrowleft")){
          this.yaw += this.rotationSpeed / this.canvas.height;
        }
        if(Input.keyHold("arrowright")){
          this.yaw -= this.rotationSpeed / this.canvas.height;
        }
        if(Input.keyHold("arrowdown")){
          this.pitch -= this.rotationSpeed / this.canvas.height;
        }
