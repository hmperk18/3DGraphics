...
class OBJLoader{
    static getMesh(gl, name, path){
        ...
        MeshCache[name] = GLUtils.createMesh(gl, name, gl.TRIANGLES, [], [], []);

        OBJLoader.loadTextFileAJAX(path,
                         function(fileContent){
                             let objData = OBJLoader.parse(fileContent)[0];
                             let tempMesh = GLUtils.createMesh(gl, "tempObjModel", gl.TRIANGLES,
                                                                objData.vertices,
                                                                objData.normals,
                                                                objData.textureCoordinates,
                                                                objData.indices);
                             let realMesh = MeshCache[name];
                             gl.deleteBuffer(realMesh.positionBuffer);
                             gl.deleteBuffer(realMesh.normalBuffer);
                             gl.deleteBuffer(realMesh.texcoordBuffer);
